dat_ctrl <- read.table("../Data/nswre74_control.txt", header=F)
...
resid <- mod$residuals
res <- permu_test_mean(groups, prediction = mod$fitted, response = dat$RE78, treatment = dat$Treated)
hist(res$perm_distribution/nrow(dat_tr)); abline(v=res$diff_means, col = "red")
d <- permu_CI_mean(groups=groups, prediction = mod$fitted, treatment = dat$Treated, response = dat$RE78, side = "both", iters=100, precision = .5, verbose = TRUE)
